...
>(({ className, ...props }, ref) => (
  <RadixAlertDialog.Portal>
    ...
    <RadixAlertDialog.Content
      ref={ref}
      className={cn(
        "fixed z-50 grid w-full max-w-sm translate-y-0 gap-4 border border-neutral-200 bg-white p-6 shadow-lg sm:rounded-lg sm:translate-y-0",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <RadixAlertDialog.Description ref={ref} className={cn("text-sm text-neutral-500", className)} {...props} />
))
